...
class AlleleSeqData():
    ...
    def __init__(self, name, count_file, fdr_file):
        ...
        try:
            self.count = pandas.read_csv(count_file,
            ...
            self.fdr = pandas.read_csv(fdr_file,
                                       AlleleSeqData._field_separator,
                                       skipfooter=4,
                                       skiprows=1,
                                       engine="python")
            ...
            for column in AlleleSeqData.CountColumns:
                assert column.value in count_columns, (
            ...
            for column in AlleleSeqData.FdrColumns:
                assert column.value in fdr_columns, (
                    "FDR data missing {} column".format(column.value)
                )
    ...
    def get_pval(self, fdr):
        ...
        """
        candidates = self.fdr.query('{} <= {}'.format(
            AlleleSeqData.FdrColumns.FDR.value, fdr))['pval']

        if candidates.empty:
            print("Warning: sample {} has no data points with FDR <= {}"
                  .format(self.name, fdr))
            return 0
    ...
    def get_candidates(self, fdr):
        ...
        candidates = self.count.query(
            '({} <= @pval) and (winning != "?")'.format(AlleleSeqData.CountColumns.P_VALUE.value))
    ...
    def get_het_snp(self, chromosome, location, reference_snp=None):
        ...
        if reference_snp:
            ...
            assert expected_reference == reference_snp, (
                "Error: reference SNP mismatch at {}:{}, expected {}, found {}"
                .format(chromosome, location, expected_reference, reference_snp)
            )
    ...
    def get_winning_allele(self, chromosome, locations, pick_min=False):
        ...
        rows = list(map(lambda location: self.get_row(chromosome, location.POS), locations))
        ...
        paternal = 0
        for row in rows:
            maternal_allele = "c{}".format(row[AlleleSeqData.CountColumns.MATERNAL_ALLELE.value])
            paternal_allele = "c{}".format(row[AlleleSeqData.CountColumns.PATERNAL_ALLELE.value])
        ...
        if (maternal > paternal) ^ pick_min:
            return row_dataframe[[AlleleSeqData.CountColumns.MATERNAL_ALLELE.value,
                                  AlleleSeqData.CountColumns.REFERENCE_SNP.value]]
        else:
            return row_dataframe[[AlleleSeqData.CountColumns.PATERNAL_ALLELE.value,
                                  AlleleSeqData.CountColumns.REFERENCE_SNP.value]]
    ...
    def __create_sequences(cls, candidates):
        ...
        for (chromosome, position), row in candidates.iterrows():
            ...
            sequence = cls.__get_sequence_from_reference_genome(chromosome, position, row)
            winning_snp = cls.__get_winning_snp(row)

            if winning_snp:
                # Mutate the sequence. Python strings are immutable so we create a copy
                sequence = (sequence[:ReferenceGenome.offset]
                            + winning_snp
    ...
    def __get_sequence_from_reference_genome(cls, chromosome, position, row):
        ...
        # Fetch the sequence from the reference genome
        sequence = cls.genome.get_peak(chromosome,
                                       position,
                                       expected_base=reference_base)
