...
def naive_mr_analysis(df_BVs, df_GWAS, GWAScodeslist, permute=False, columns_bvs={}, usecols_gwas={}, min_MAF=1.0e-3, min_HWE=1.0e-50,  min_iscore=0.9):
    """ Perform naive MR analysis """

    # define output format
    out = []
    out_cols = list(df_BVs.columns) + list(df_GWAS.columns) + \
    ...
    for _, BV in df_BVs.iterrows():
        ...
        rsid_GWAS = df_GWAS.loc[df_GWAS[usecols_gwas['rsid']]
                                == BV[columns_bvs['rsid']]]
        ...
        if not rsid_GWAS.empty:
            ...
            rsid_GWAS = rsid_GWAS.loc[(rsid_GWAS[usecols_gwas['MAF']] >= min_MAF) & (
                rsid_GWAS[usecols_gwas['HWE']] >= min_HWE) & (rsid_GWAS[usecols_gwas['iscore']] >= min_iscore)]

            # Permute all the rsids in the GWAS results
            if permute:
                rsid_GWAS[usecols_gwas['rsid']] = np.random.permutation(
                    rsid_GWAS[usecols_gwas['rsid']])
            ...
            for _, trait_GWAS in rsid_GWAS.iterrows():
                ...
                if trait_GWAS[usecols_gwas['trait']] in GWAScodeslist:

                    # determine effect allele
                    if trait_GWAS[usecols_gwas['allele']] == BV[columns_bvs['alt']]:
                        allele_swap_sign = 1
                    elif trait_GWAS[usecols_gwas['allele']] == BV[columns_bvs['ref']]:
                        allele_swap_sign = -1
                    else:
                        allele_swap_sign = 0
                    ...
                    if allele_swap_sign != 0:
                        causal_es, causal_se = causal_effect(
                            BV[columns_bvs['es']], BV[columns_bvs['es_sterr']], trait_GWAS[usecols_gwas['beta']], trait_GWAS[usecols_gwas['NSE']])
                        z = causal_es/causal_se
                        p = norm.sf(abs(z))*2.
                        out.append(list(BV) + list(trait_GWAS) +
                                   [causal_es, causal_se, z, p])
        ...
        # output nan if rsid in BV does not exist in GWAS results
            ...
            out.append(list(
                BV) + [np.nan for i in range(len(list(df_GWAS.columns)))] + list(np.full(4, np.nan)))
