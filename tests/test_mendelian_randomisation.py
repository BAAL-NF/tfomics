...
def test_causal_effect_no_gwas_effect():
    ...
    assert (0.0, 1.0) == mr._calculate_causal_effect(2.0, 22.1, 0.0, 2.0)

    # Error scales linearly with gwas_error
    assert (0.0, 4.0) == mr._calculate_causal_effect(1.0, 22.1, 0.0, 4.0)


def test_causal_effect_full():
    """Test causal effect with gwas_effect != 0"""
    assert (3.0, 5.0) == mr._calculate_causal_effect(1.0, 1.0, 3.0, 4.0)
...
def test_singularity_if_exposure_is_zero():
    ...
    with pytest.raises(ZeroDivisionError):
        mr._calculate_causal_effect(0.0, 23.0, 3.12, 2.17)
